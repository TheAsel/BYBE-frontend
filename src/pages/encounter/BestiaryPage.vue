...
const changeVariant = (variant: variants) => {
  if (creatureVariant.value === 'Base') {
    const routeData = router.resolve({
      name: 'bestiary',
      query: { game: currentGame.value, id: creatureId }
    });
    globalThis.open(routeData.href, '_self');
  } else {
    const routeData = router.resolve({
      name: 'bestiary',
      query: { game: currentGame.value, id: creatureId, variant: variant.toLowerCase() }
    });
    globalThis.open(routeData.href, '_self');
  }
};
    ...
    >
      <q-scroll-area style="height: calc(100vh - 128px)">
        <div class="q-gutter-y-xs tw:p-4 show-print">
          ...
          >
            <a
              v-if="currentFont === 'pf' && creatureData?.core_data.derived.archive_link"
              class="tw:my-auto"
              :href="
            ...
            <a
              v-else-if="currentFont === 'sf' && settings.getAonLinks && creatureData"
              class="tw:my-auto"
              :href="
